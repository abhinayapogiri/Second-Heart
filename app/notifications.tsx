...
export default function NotificationsScreen() {
    ...
    const renderItem = ({ item }: { item: Notification }) => (
        <View style={[styles.notificationCard, !item.is_read && styles.unreadCard]}>
            <View style={[styles.iconContainer, { backgroundColor: item.title.includes('Confirm') ? 'rgba(34, 197, 94, 0.1)' : 'rgba(138, 43, 226, 0.1)' }]}>
                {item.title.includes('Confirm') ? (
...
const styles = StyleSheet.create({
    ...
    list: { paddingHorizontal: width * 0.05, paddingBottom: 40 },
    notificationCard: {
        flexDirection: 'row',
